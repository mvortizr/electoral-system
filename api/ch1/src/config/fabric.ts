...
export const getContracts = async (
  network: Network
): Promise<{ positionContract: Contract; qsccContract: Contract }> => {
  const positionContract = network.getContract(config.chaincodeName);
  const qsccContract = network.getContract('qscc');
  return { positionContract, qsccContract };
};
