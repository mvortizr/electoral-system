...
function   construct_rest_configmap_ch3() {
  local ns=$ORG1_NS
  ...
  CONFIG_DIR=${TEMP_DIR}/channel3-config
  CHAINCODE_NAME="$1"
...
function rollout_rest_api_ch3() {
  local ns=$ORG1_NS
  push_fn "Starting rest_api_ch3"

  kubectl -n $ns apply -f kube/ch3-api.yaml
  kubectl -n $ns rollout status deploy/ch3-api

  pop_fn
}

function bring_down_rest_api_ch3() {
  local ns=$ORG1_NS
...
function launch_rest_api_ch3() {
  local ns=$ORG1_NS  
  # push_fn "this is number one $1"
  # pop_fn
  construct_rest_configmap_ch3 $1

  apply_template kube/channel3-api.yaml $ns

  kubectl -n $ns rollout status deploy/channel3-api
